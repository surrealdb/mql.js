 ...
 *
 * Records every call (`query`, `createRecord`, `insertMany`, `close`) and
 * returns a programmable queue of responses for `query`. Lets us assert
 * exactly which SurrealQL strings + bindings the operation modules
 * produce without spinning up a real database.
 */
...
export interface RecordedCreate {
	recordId: RecordIdLike;
	content: Document;
}

export interface RecordedInsertMany {
	table: Table | string;
	docs: Document[];
}
...
export class FakeQueryExecutor implements QueryExecutor {
	...
	readonly insertManyCalls: RecordedInsertMany[] = [];
	closed = false;
	...
	async createRecord(recordId: RecordIdLike, content: Document): Promise<void> {
		this.createCalls.push({ recordId, content });
	}

	async insertMany(table: Table | string, docs: Document[]): Promise<void> {
		this.insertManyCalls.push({ table, docs });
	}
