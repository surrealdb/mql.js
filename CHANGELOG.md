...
- **`$group` accumulators `$stdDevSamp`, `$stdDevPop`, `$firstN`, `$lastN`, `$maxN` and `$minN`.** `$stdDevSamp` is `math::stddev` directly — SurrealDB only has the sample statistic, verified live by reconstructing the population and sample formulas from the same input and checking which one `math::stddev`/`math::variance` agree with. `$stdDevPop` has no counterpart, so it is derived from the sample one by the standard scaling factor, `sqrt((n-1)/n)`, with both sides of the division cast to `<float>` — SurrealQL's `/` on two integers truncates, which silently made every derived population value `0` before the cast was added. `$firstN`/`$lastN` slice `array::group`'s result from the front or back; `$maxN`/`$minN` sort it first, descending and ascending respectively, matching the order MongoDB documents for each.

- **Expression operators `$reduce`, `$objectToArray`, `$arrayToObject`, the `$set` family (`$setUnion`, `$setIntersection`, `$setDifference`, `$setEquals`, `$setIsSubset`), `$dateAdd`, `$dateDiff`, `$dateTrunc`, `$replaceAll` and `$convert`.**

  `$reduce` compiles to `array::reduce`, whose closure seeds itself from its array's first element rather than taking a separate initial value — so `initialValue` is prepended with `array::concat` before reducing, which folds to the same answer including over an empty `input`. Its closure needs two parameters in one header, `|$value, $this|`, which the existing single-variable closure machinery could not produce; a new `withClosureVariables` mints both from the same synthetic-name counter the rest of the file already uses, so a `$reduce` inside a `$map` still cannot collide.

  `$objectToArray`/`$arrayToObject` bridge MongoDB's `{k, v}` document shape and SurrealDB's `[k, v]` pair, in both directions: `object::entries` returns pairs, remapped into documents for `$objectToArray`; `$arrayToObject` accepts either shape MongoDB does, telling them apart per element at runtime with `type::is_array`, since which one a caller used is not known when this compiles.

  The `$set` family treats arrays as sets. `array::union`/`array::intersect` already de-duplicate; SurrealDB's one-directional difference, `array::complement`, does not — verified live, `complement([1,1,2,5], [2,3])` kept both `1`s — so it is wrapped in `array::distinct`. `$setIsSubset(a, b)` is `b CONTAINSALL a`.

  `$dateAdd`/`$dateDiff`/`$dateTrunc` are scoped to units with a fixed length: millisecond, second, minute, hour, day, and week for `$dateAdd` alone. `month`, `quarter` and `year` have no fixed duration to construct; `$dateDiff`/`$dateTrunc`'s `week` additionally needs a `startOfWeek`-relative calendar boundary SurrealQL has no equivalent for, so both refuse it where `$dateAdd` — adding a fixed seven days — does not need one. SurrealQL durations are unsigned (`1h * -5` is a runtime error, not negative five hours), so a negative `$dateAdd` amount is handled by subtracting the magnitude, and `$dateDiff`'s direction is branched on rather than subtracted-and-possibly-negative, both verified live. `$dateTrunc` is `time::floor`, not `time::round` — `time::round` rounds to the *nearest* bin, verified live to round `13:47:32` at a one-hour bin *up* to `14:00`, where truncation means down. `timezone` and `startOfWeek` are refused outright, for the reason `$dateToString`'s `timezone` already is: every value here is UTC.

  `$replaceAll` is `string::replace`, which already replaces every occurrence rather than the first. `$convert` is scoped to `to: "string" | "bool" | "int" | "double"`, the same four casts `$toString`/`$toBool`/`$toInt`/`$toDouble` already provide; `onError`/`onNull` are refused, since SurrealQL has no try/catch to catch a failing cast with.

### Fixed

- **`$project`/`$unset` excluding a field other than `_id` silently did nothing.** The exclusion path OMITted a column literally named `_id` — which does not exist on a stored row, the column is `id` — so `{$project: {secret: 0}}` built `* OMIT \`_id\`` against a table with no such column, omitted nothing, and returned `secret` anyway. `$project`'s exclusion now runs a general `projectByExclusion`, shared with `$unset`, that OMITs the fields actually named and marks the stage as *not* reshaping the identity column — the previous code marked every `$project` with an exclusion as reshaped regardless of whether it renamed anything, which is what let the wrong OMIT target go unnoticed. `{_id: 1}` alone is still read as "the document is just `_id`", not swallowed by the exclusion branch as a no-op.

- **`find()`'s exclusion projection ran in JavaScript, fetching every field over the wire and deleting the excluded ones from the returned document.** It now runs the same `OMIT` idiom `$lookup`/`$graphLookup` already use for their own projections, server-side, so an excluded field is never sent rather than sent and discarded. `recordToDocument` also now omits `_id` entirely rather than setting it to `undefined` when a row has no `id` — `"_id" in doc` is `false` rather than `true`, matching a document that never had the key.
